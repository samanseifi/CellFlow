...
\paragraph{Diffusion.} An explicit five-point (FTCS) scheme is used,
\begin{equation}
  c^{m+1}_{ij} = c^{m}_{ij}
  + \frac{D_c \dt}{\dx^2}\left(c_{i-1,j}+c_{i+1,j}+c_{i,j-1}+c_{i,j+1}-4c_{ij}\right),
\end{equation}
clamped to be non-negative. It is stable when the diffusion number
$D_c\dt/\dx^2 \le 1/4$; the code warns when this is violated. Under no-flux
boundaries the discrete scheme is conservative in the interior (verified) but
the node-centred ghost treatment is not strictly flux-conservative at the walls.
...
are removed by a sequential geometric projection (each overlapping pair is
pushed apart by half the overlap), iterated a configurable number of times per
step. (The degenerate exactly-coincident case is separated along a deterministic
golden-angle direction, preserving reproducibility.)
...
\begin{itemize}
  ...
  \item \textbf{Diffusion:} explicit FTCS; stable for $D\dt/\dx^2 \le 1/4$.
...
CellFlow ships an automated test suite (87 tests) including:
\begin{itemize}
  ...
  \item \textbf{Physics checks:} screening monotonicity; mechanotransduction
        ...
        isotropic stress; area-conserving division.
...
\begin{itemize}
  \item Two spatial dimensions only; periodic (Brinkman) boundaries with
        screening as a confinement surrogate (no explicit no-slip walls yet).
