...
\paragraph{Cell-scale surface tension bounds interfacial patterning.}
This one is worth stating explicitly because it constrains a whole class of
studies. In a soft-particle model the effective surface tension originates in
cell-scale sterics (repulsion plus the overlap re-rounding projection), so the
capillary length $d_0$ cannot fall far below one cell radius $a$. Measured via
$d_0\sim R/k_0^2$ (Section~\ref{sec:front}), it is $3$--$8\,a$ across the regimes
tested, and it is \emph{larger} when the colony advances by proliferation than
when cells merely swell. Combined with $k^\ast\sim\sqrt{R/d_0}$, the number of
fingers a colony can support grows only as the square root of its radius:
\begin{equation}
  k^\ast \sim \sqrt{R/d_0} \;\lesssim\; \sqrt{R/3a}.
\end{equation}
Resolving a strongly branched morphology ($k^\ast\sim 10$--$30$) therefore
requires $10^5$--$10^7$ cells. Interfacial instabilities are not forbidden in this
model class, but at accessible colony sizes only the longest-wavelength modes
($k\lesssim 5$) are unstable, which produces a lumpy or elongated colony rather
than fingers. See \texttt{docs/giverso\_replication.md} for the measurements.

\paragraph{Growth-driven expansion.} The volumetric source
(Section~\ref{sec:growthsource}) carries three caveats: the source mean is
subtracted (a uniform far-field drain, accurate only while the colony is small
compared with the box); it is applied with a one-step lag; and it double-counts
expansion against the overlap projection unless
\texttt{overlap\_iterations} or \texttt{growth\_source\_strength} is reduced.
Growth-driven flow can also be fast enough to violate the advective CFL condition
--- check the reported CFL when enabling it.
